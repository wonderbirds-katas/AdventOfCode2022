...
{
    public static Rucksack PackRucksackWith(
    ...
    private static void AddItemsToCompartment(
        char[] itemsInFirstCompartment,
        Compartment compartment
    )
    {
        if (itemsInFirstCompartment.Length == 0)
            return;

        Array.ForEach(itemsInFirstCompartment, itemType => compartment.Add(new Item(itemType)));
