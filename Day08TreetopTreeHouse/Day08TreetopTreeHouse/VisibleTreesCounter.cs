...
public class VisibleTreesCounter
{
    ...
    private VisibleTreesCounter(IEnumerable<string> treeHeightGrid) =>
        _trees = Matrix<Tree>.FromList(
            treeHeightGrid.ToList(),
            digit => new Tree(int.Parse(digit), false)
        );

    public static int Count(IEnumerable<string> treeHeightGrid) =>
        new VisibleTreesCounter(treeHeightGrid).Count();
        ...
        )
        {
            ...
            )
            {
                if (dimension2Iter.height > largest)
                {
                    ...
                    if (innerDimensionIsReversed)
                    {
                        inner = dimension1Iter.dimension2.Count() - 1 - inner;
                    }
                    ...
                    _trees.SetValue(row, col, new Tree(dimension2Iter.height, true));
